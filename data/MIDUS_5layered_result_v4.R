...
################################################################################
setwd("/Users/hyunseokyoon/Desktop/학교/대학원/Research/joint_LSIRM/data/MIDUS_2")
source("MIDUS_EDA_2_v3.R")

# 5-layered LSIRM 모델 로드 (v4: unified gamma)
setwd("/Users/hyunseokyoon/Desktop/학교/대학원/Research/joint_LSIRM")
source("data/MIDUS_2/my_LSIRM_5layered_nonhierarchical_v4_cpp.R")
source("utils.R")
...
# plot 디렉토리 생성
plot_dir <- "/Users/hyunseokyoon/Desktop/학교/대학원/Research/joint_LSIRM/data/MIDUS_2/plot_v4"
if (!dir.exists(plot_dir)) dir.create(plot_dir, recursive = TRUE)


################################################################################
# 2. Analysis Case 1: P1–P4
################################################################################
cat("\n\n========== Case 1: P1-P4 ==========\n")
...
Y_ord1_p4 <- lsirm_p4$Y_ord1
Y_ord2_p4 <- lsirm_p4$Y_ord2
...
result_p4 <- lsirm_sharedpos_layer5_unified_gamma_cpp(
...
result_p3 <- lsirm_sharedpos_layer5_unified_gamma_cpp(
...
result_all <- lsirm_sharedpos_layer5_unified_gamma_cpp(
...
make_traceplots <- function(result, prefix, lsirm_data) {
  ...
  if (!is.null(res$samples$log_gamma))      plot_trace_scalar(res$samples$log_gamma,      true = NA, main = "gamma (unified)", transform = exp)
...
# ── Case 1: P1-P4 ──
# make_traceplots(result_p4, prefix = "v4_P4", lsirm_data = lsirm_p4)

# ── Case 2: P1-P3 ──
# make_traceplots(result_p3, prefix = "v4_P3", lsirm_data = lsirm_p3)

# ── Case 3: P1-P3-P4 ──
make_traceplots(result_all, prefix = "v4_ALL", lsirm_data = lsirm_all)
...
make_biplot <- function(result, lsirm_data, title, filename) {
  ...
  has_bin  <- length(lsirm_data$col_bin)  > 0
  has_con  <- length(lsirm_data$col_con)  > 0
  has_cnt  <- length(lsirm_data$col_cnt)  > 0
  has_ord1 <- length(lsirm_data$col_ord1) > 0
  has_ord2 <- length(lsirm_data$col_ord2) > 0
  ...
  resp_legend <- if (has_branch) c("Sadness only", "Anhedonia only", "Both") else "Respondent"
  resp_bg <- if (has_branch) c("#E41A1C", "#377EB8", "#984EA3") else "gray80"
  resp_pch <- if (has_branch) c(21, 21, 24) else 21

  item_legend <- c(
    if(has_bin)  "Binary (med/health)" else NULL,
    if(has_con)  "Continuous (bio/cog)" else NULL,
    if(has_cnt)  "Count (sleep/drink)" else NULL,
...
# make_biplot(result_p4,  lsirm_p4,  "v4: P1-P4 (Bio+Sleep)",  "v4_P4_biplot.pdf")
# make_biplot(result_p3,  lsirm_p3,  "v4: P1-P3 (Cognitive)",  "v4_P3_biplot.pdf")
make_biplot(result_all, lsirm_all, "v4: P1-P3-P4 (Unified Gamma)", "v4_ALL_biplot.pdf")
...
if (length(lsirm_all$col_bin) > 0) {
  ...
  for (j in seq_len(n_bin)) {
    ...
    barplot(mat, beside = TRUE, col = c("#E41A1C", "#377EB8"),
            main = lsirm_all$col_bin[j], xlab = "Response", ylab = "Proportion", ylim = c(0, 1))
