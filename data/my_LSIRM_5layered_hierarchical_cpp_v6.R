...
sourceCpp(file.path(getwd(), "my_LSIRM_5layered_hierarchical_v6.cpp"))

lsirm_hierarchical_layer5_grm_cpp <- function(
  ...
  if (is.null(init)) {
    # GRM thresholds: P x (K-1) matrix, descending order per row
  ...
  # --- Run C++ MCMC ---
  cat("Running C++ MCMC (5-layered hierarchical, GRM ordinal, robust continuous, nu2 =", nu2, ")...\n")
  res <- run_lsirm_v6_cpp(
    Y_bin, Y_con, Y_cnt, Y_ord1, Y_ord2,
  ...
  get_stacked_coords <- function(idx) {
    parts <- list(Z_arr[,,idx], A1_arr[,,idx], A2_arr[,,idx], A3_arr[,,idx], A4_arr[,,idx], A5_arr[,,idx])
  ...
  idx_Z  <- 1:n;                     offset <- n
  idx_A1 <- (offset+1):(offset+n);   offset <- offset + n
  idx_A2 <- (offset+1):(offset+n);   offset <- offset + n
  idx_A3 <- (offset+1):(offset+n);   offset <- offset + n
  idx_A4 <- (offset+1):(offset+n);   offset <- offset + n
  idx_A5 <- (offset+1):(offset+n);   offset <- offset + n
  ...
  for (i in 1:(n_save-1)) {
    ...
    res$z[,,i]  <- Aligned[idx_Z, ]
    res$a1[,,i] <- Aligned[idx_A1, ]
    res$a2[,,i] <- Aligned[idx_A2, ]
    res$a3[,,i] <- Aligned[idx_A3, ]
    res$a4[,,i] <- Aligned[idx_A4, ]
    res$a5[,,i] <- Aligned[idx_A5, ]
  ...
  res$z  <- aperm(res$z,  c(3,1,2))
  res$a1 <- aperm(res$a1, c(3,1,2))
  res$a2 <- aperm(res$a2, c(3,1,2))
  res$a3 <- aperm(res$a3, c(3,1,2))
  res$a4 <- aperm(res$a4, c(3,1,2))
  res$a5 <- aperm(res$a5, c(3,1,2))
  ...
  if (!is.null(res$beta4)) res$beta4 <- aperm(res$beta4, c(3,1,2))  # (n_save, P4, K1-1)
  if (!is.null(res$beta5)) res$beta5 <- aperm(res$beta5, c(3,1,2))  # (n_save, P5, K2-1)
  if (!is.null(res$lambda2)) res$lambda2 <- aperm(res$lambda2, c(3,1,2))  # (n_save, n, P2)
  res$info <- list(n=n, P1=P1, P2=P2, P3=P3, P4=P4, P5=P5, K1=K1, K2=K2, nu2=nu2)
  return(res)
}
