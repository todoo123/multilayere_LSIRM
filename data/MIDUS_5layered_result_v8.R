...
################################################################################
# v8: Joint multilayered LSIRM + Factor-Mixture Clustering on MIDUS
#
# Single chain with default hyperparameters (r_fac = 4, K_star = 10, e0 = 0.05).
#
# IMPORTANT NOTES
#   - Coupling is strict ONE-WAY (LSIRM -> FMC). The LSIRM block in
#     my_LSIRM_FMC_v8.cpp is byte-identical to v6; clustering does NOT
...
################################################################################
setwd(data_dir)
source(file.path(data_dir, "my_LSIRM_FMC_cpp_v8.R"))   # joint LSIRM + FMC v8
...
fmc_init_smart <- list(
  ...
  Lambda  = matrix(rnorm(n_init * r_fac, 0, 0.1), n_init, r_fac),
  delta   = rep(0, n_init),
  psi     = rep(1, n_init),
  sigma_delta_sq = 0.1
)
...
# 4. Single chain run
################################################################################
run_label     <- sprintf("v8_fmc_r%d_K%d_e%g", r_fac, K_star, e0)
...
if (common_mcmc$d == 2 && nrow(B_hat_pm) >= 1) {
  make_fmc_biplot(
    ...
    title = sprintf("Joint LSIRM+FMC v8  (r=%d, K*=%d, e0=%g, mean K_+=%.1f)  |  %s",
...
write.csv(data.frame(respondent     = seq_along(result$fmc_psi_postmean),
                     psi_postmean   = result$fmc_psi_postmean,
