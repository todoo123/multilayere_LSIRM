...
################################################################################
# v11: Joint multilayered LSIRM + LATENT-POSITION Gaussian-mixture clustering
#       on MIDUS
#
...
# 0506 LOOSE-B variant (option B in history/0506_history.md):
#   Same as LOOSE-SIGMA (nu0 = 10 to allow data-driven Sigma_l) but with
#   the prior MEAN of Sigma_l RAISED to match the LSIRM-natural b spread.
#
#   Motivation: LOOSE-SIGMA (target 0.062 * I, per-dim sd 0.25) gave
#   strong PSM contrast (PEAR 0.42, within-pair 0.45) but the biplot
#   showed b positions pulled toward mu_c (geometry distortion).  The
#   v12 EPA reference run (cluster prior absent in b update) showed
#   silhouette ~0.42 with cluster-center spread ~1, within-cluster sd
#   ~0.5, suggesting LSIRM's natural b within-cluster spread is ~0.5
#   per dim, not 0.25.  So 0.25 was simply too tight a target -- the
#   mixture pulled b toward mu_c.
#
#   This variant raises the target uniformly across c, mu, Sigma, b
#   updates (model-coherent; not a sampler hack like b_prior_inflation):
#       target E[Sigma_l] = 0.25 * I  (per-dim sd 0.5)
#       nu0 = 10  (unchanged)
#       S0  = 0.25 * (nu0 - d - 1) * I = 0.25 * 7 * I = 1.75 * I
#
#   Expected effect:
#     - b geometry: closer to LSIRM-only (less pull toward mu_c)
#     - PSM contrast: slightly weaker than LOOSE-SIGMA (looser within-
#       cluster prior tolerates more boundary-item swap), but should
#       remain stronger than v12 EPA.
#     - K_+ posterior: probably unchanged (still 4 mode).
#
# Risk: with looser within-cluster prior, the chain may merge clusters
# more aggressively because the within-cluster variance can absorb
# heterogeneous items.  If median K_+ drops to 2 or 3, the target was
# raised too much -- step back to 0.15 * I.
#
# kappa0 unchanged at 0.1.  At target Sigma_l = 0.25 * I, mu_l prior
# sd = sqrt(0.25 / 0.1) = 1.58 -- cluster centers can span +-3, very
# permissive.  This may be looser than needed but is fine since the
# posterior on mu_l is data-driven.
S0_init_scale <- 1.75
common_fmc_hyper <- list(
  e0     = e0,
  m0     = rep(0, d),
  kappa0 = 0.1,                          # unchanged
  nu0    = 10,                           # unchanged from LOOSE-SIGMA
  S0     = S0_init_scale * diag(d),      # E[Sigma_l] = 1.75/7 = 0.25 * I (per-dim sd 0.5)
  nu_S0  = NA                            # Wishart hyperprior on S0 still off
)

# v11 b-prior inflation knob (sampler-level, NOT a model change).
# 0506 reverted to 1.0 because the runaway feedback that earlier motivated
# alpha > 1 is now addressed at its root via the pinned Sigma_l above.
# alpha = 1 keeps the paper-defined full joint coupling.
...
# 4. Single chain run
################################################################################
run_label     <- sprintf("v11_fmc_d%d_K%d_e%g_S0init_%g_nu%d_M%d_alpha%g",
                         d, K_star, e0,
                         S0_init_scale,
...
result <- lsirm_fmc_v11_cpp(
  ...
  b_prior_inflation = b_prior_inflation,
  verbose       = TRUE,
  fix_gamma     = FALSE
)
...
if (common_mcmc$d == 2 && nrow(B_hat_pm) >= 1) {
  make_fmc_biplot(
    ...
    title = sprintf("Joint LSIRM + latent-position FMC v11  (d=%d, K*=%d, e0=%g, mean K_+=%.1f, split=%.2f, alpha=%g)  |  %s",
                    d, K_star, e0,
...
cat(sprintf("  b_prior_inflation = %g, S0 hyperprior = %s\n",
            b_prior_inflation, isTRUE(result$fmc_S0_hyperprior)))
...
print(table(P5_dahl))
cat(sprintf("\n-> v11 plots & artifacts saved to: %s\n", case_plot_dir))
