...
#
# eta_ij = alpha + theta_i + theta_j - beta * d(z_i, z_j)
#
# Priors:
# i_ij ~ Bernoulli(p)
# p ~ Beta(a_p, b_p)
...
run_mixture_lsm_mcmc <- function(W, 
  ...
  for (iter in 1:n_iter) {
    ...
    # 현재 Z 기준 거리/eta 계산
    dist_mat <- L2_dist(Z)
    eta_mat <- calculate_eta_matrix(alpha_curr, theta_curr, beta_curr, dist_mat)
    eta_vec <- eta_mat[upper_tri_indices]
    ...
    eta_mat_prop <- calculate_eta_matrix(alpha_curr, theta_curr, beta_prop, dist_mat)
    ...
    for (i in 1:N) {
      ...
      eta_mat_prop <- calculate_eta_matrix(alpha_curr, theta_curr, beta_curr, dist_mat_prop)
      eta_vec_prop <- eta_mat_prop[upper_tri_indices]
    ...
    if (iter > burn_in && (iter - burn_in) %% thin == 0) {
      ...
      samples$ll_z[save_idx] <- calc_log_post_z(Z, Y_vec, upper_tri_indices, I_idx_1,
                                        alpha_curr, theta_curr, beta_curr,
...
res <- run_mixture_lsm_mcmc(R_eff, n_iter = 20000, burn_in = 5000, thin = 1)
...
sum(colMeans(res$samples$I_mat, dims = 1) < 0.5)

colMeans(res$samples$I_mat, dims = 1)

dim(res$samples$I_mat)
