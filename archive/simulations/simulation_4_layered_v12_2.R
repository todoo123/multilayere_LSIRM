...
#
#    v12_2 (this file):
#      - Same model and same MCMC settings as simulation_4_layered_v12.R.
#      - The ONLY change is the data-generating noise scale:
#          * sigma_meta diagonals shrunk from {0.15, 0.20, 0.10, 0.18}
#            to {0.05, 0.07, 0.03, 0.06}  (~1/3 of v12).
#          * sd_cluster_resp shrunk from 0.5 to 0.3.
#        Cluster centers (+/-1, +/-1) are unchanged, so the four item
#        meta-clusters are now much more clearly separated in the latent
#        space.  Use this as an "easy mode" sanity check for v12; if the
#        sampler cannot recover clusters here, no parameter tweak on the
#        harder v12 setting will help.
# =========================================================
...
# v12_2: tighter item-cluster covariances (~1/3 of v12) so the four
# meta-clusters in the latent space are more clearly separated.  Cluster
# centers are unchanged (+/-1, +/-1), so the inter-center distance is still 2,
# but typical within-cluster std dev drops from ~0.32-0.45 to ~0.17-0.26.
sigma_meta <- list(
  diag(c(0.05, 0.05)),
  diag(c(0.07, 0.07)),
  diag(c(0.03, 0.03)),
...
# v12_2: respondent cloud also tightened (0.5 -> 0.3) so respondent-side
# heterogeneity does not blur item-cluster recovery.
sd_cluster_resp <- 0.3
...
if (nchar(sens_label) > 0) {
  ...
  plot_dir <- file.path(
    proj_dir, "plot",
    sprintf("simulation_4_layered_v12_2_sens_%s", exp_label))
} else {
  ...
  plot_dir <- file.path(
    proj_dir, "plot",
    sprintf("simulation_4_layered_v12_2_d%d_sigb_%g_exp%s",
            d, sigma_b, exp_label))
}
...
# =========================================================
cat(sprintf(
  "\n========== simulation_4_layered_v12_2 [easy/tight clusters, exp %s] (n=%d, P=%d/%d/%d/%d, K_true=%d, sigma_b=%g, M_SM=%d, R=%d, M_perm=%d, update_alpha=%s, update_tau=%s) ==========\n",
...
cat(sprintf("\n-> v12_2 (easy/tight clusters) simulation plots & artifacts saved to: %s\n", plot_dir))
cat("\n=== Done! ===\n")
