...
# =========================================================
# 0) Compile / source v9 wrappers
#
#    v9 model (joint LSIRM + PPCA-FMC, MIDUS_5layered_result_v9.R style):
#      - Non-hierarchical 5-layer LSIRM (single shared a_i across layers,
#        per-layer items b_l_j, per-layer alpha_l_i and gamma_l, per-item
#        log_kappa for the count layer, GRM thresholds for ord layers)
#      - PPCA-style mixture clustering on item-level latent factors
#        eta_j (j = 1..P_total), with one-way coupling LSIRM -> FMC.
#
#    Simulation:
#      - 4-layer simulation (binary / continuous / count / ordinal).
#      - We use the 5-layer model wrapper but leave layer 5 EMPTY,
#        matching the MIDUS workflow.
#      - Items across all 4 layers are partitioned into K_true item
#        meta-clusters; items within a meta-cluster get nearby
#        positions in the LSIRM latent space, so the FMC should
#        recover the meta-clusters from log-distances.
# =========================================================
...
d        <- 2L
r_fac    <- 10L     # FMC latent factor dimension (matches MIDUS v9)
K_star   <- 10L    # FMC overfitted upper bound
e0       <- 1.0    # small e0 -> Rousseau-Mengersen emptying (with PCA init)
K_true   <- 4L     # number of TRUE item meta-clusters

K1       <- 5L     # ordinal categories

gamma_true     <- 1.0
sigma0_sq_true <- 1.0
kappa_true     <- 1.0    # NB overdispersion (shared scalar; per-item in fit)
nu2_true       <- 5      # generation df (Student-t)
nu2_fit        <- 4      # df used in fitting (matches MIDUS)

# =========================================================
# 2) TRUE item meta-clusters in LSIRM latent space
#
#    K_true = 4 clusters arranged in a "diamond" with centers
#    closer together so 2-3 sigma envelopes touch (overlapping
#    boundaries). Cluster shapes are mixed: round + elliptical
#    (axis-aligned and rotated) to test whether the FMC recovers
#    non-isotropic Gaussian components via Sigma_l.
# =========================================================
centers_meta <- rbind(
  c(-1.0,  1.0),    # cluster 1: round
  c( 1.0,  1.0),    # cluster 2: elliptical (wide horizontally)
  c( 1.0, -1.0),    # cluster 3: round
  c(-1.0, -1.0)     # cluster 4: elliptical, rotated 45 deg
)
stopifnot(nrow(centers_meta) == K_true, ncol(centers_meta) == d)

# Per-cluster covariance matrices (2x2). Designed so 2 sigma
# envelopes from neighbouring clusters touch around the origin.
make_rot <- function(theta) {
  cs <- cos(theta); sn <- sin(theta)
  matrix(c(cs, -sn, sn, cs), 2, 2)
}
sigma_meta <- list(
  diag(c(0.18, 0.18)),                                                 # round
  diag(c(0.45, 0.07)),                                                 # wide ellipse along x
  diag(c(0.18, 0.18)),                                                 # round
  make_rot(pi / 4) %*% diag(c(0.45, 0.07)) %*% t(make_rot(pi / 4))     # 45-deg rotated ellipse
)

# Each layer cycles through the meta-clusters so every layer
# contributes items to every cluster (avoids confounding cluster
# with layer).
...
true_item_cluster <- c(B1_meta, B2_meta, B3_meta, B4_meta)   # length P_total

# Respondents: spread over the same 4 cluster centres for visual
# coherence (FMC does NOT cluster respondents in v9).
...
# Sample items around per-cluster centres, with per-cluster covariance.
# centers       : K x d
# cluster_ids   : length-P vector of cluster ids in 1..K
# sigma_list    : length-K list of d x d positive-definite covariances
...
# =========================================================
# 4) TRUE positions: respondents (a) + per-layer items (b1..b4)
# =========================================================
...
# =========================================================
# 6) Linear predictors per layer (single shared a, layer-specific b)
# =========================================================
...
# Layer 2: Student-t robust continuous
...
# Layer 3: Negative binomial count
...
# Empty 5th layer (5-layer model, only 4 used)
...
# Item names (consistent with how MIDUS_5layered_result_v9.R concatenates)
...
# =========================================================
plot_dir <- file.path(proj_dir, "plot", "simulation_4_layered_v9")
...
plot_positions_v9 <- function(A, B_list, B_meta_list, A_resp_cl,
                              title = "TRUE latent positions (4-layer, v9)",
...
# =========================================================
# 9) FMC PCA-smart initialisation (mirrors MIDUS_5layered_result_v9.R)
# =========================================================
...
# =========================================================
# 11) Run v9 joint MCMC
# =========================================================
cat(sprintf(
  "\n========== simulation_4_layered_v9 (n=%d, P=%d/%d/%d/%d, K_true=%d, K*=%d) ==========\n",
  n, P1, P2, P3, P4, K_true, K_star))
...
# Per-item kappa traceplots (count layer)
...
# =========================================================
# 17) Posterior mean LSIRM positions + true vs estimated plot
# =========================================================
...
# =========================================================
# 18) Distance recovery (per layer, log-distance scale)
# =========================================================
...
#
#     Per-iteration labels suffer label switching, so we use:
#       - posterior similarity matrix (online co-cluster) for partition
#       - mode label (display only)
#       - ARI(final_partition, true_item_cluster) as the recovery metric
# =========================================================
...
# ARI helper (no extra package needed)
...
# Co-cluster heatmap reordered by TRUE clusters (block structure should appear)
...
axis(2, at = seq_len(P_total), labels = item_names_full[ord_true],
     las = 2, cex.axis = 0.55)
# divider lines at true-cluster boundaries
...
# Co-cluster heatmap reordered by FMC posterior partition
...
# eta posterior-mean scatter, coloured by TRUE and by FMC-mode
...
# =========================================================
# 21) Biplot — LSIRM (a, b) coloured by FMC partition + TRUE cluster
# =========================================================
...
# 22) CSV / RDS outputs
# =========================================================
saveRDS(result, file.path(plot_dir, "v9_simulation_result.rds"))
...
cat(sprintf("\n-> v9 simulation plots & artifacts saved to: %s\n", plot_dir))
cat("\n=== Done! ===\n")
