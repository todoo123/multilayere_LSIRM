...
# =========================================================
# 0) Compile / source v11t wrappers
#
#    v11t model (joint LSIRM + latent-position Student-t mixture):
#      - LSIRM block byte-identical to v11 (5 layers: bin/con/cnt/ord1/ord2,
#        per-item kappa, robust Student-t continuous, GRM ordinal).
#      - Mixture changed from Gaussian to Student-t (Gaussian scale-mixture
#        form, Lange-Little-Taylor 1989):
#          b_q | c_q = l ~ t_{nu_t}(mu_l, Sigma_l)
#        equivalently
#          w_q ~ Gamma(nu_t/2, nu_t/2)
#          b_q | c_q = l, w_q ~ N_d(mu_l, Sigma_l / w_q).
#      - b_j MH ratio prior is now log N_d(b_j; mu_{c_q}, Sigma_{c_q}/w_q).
#      - NIW posterior uses WEIGHTED sufficient statistics (kappa_n includes
#        sum w_q, weighted scatter, etc.).
#      - Per-sweep w_q Gibbs update from Gamma full conditional.
#      - Replaces v11's b_prior_inflation knob with a model-coherent
#        Student-t mixture (no sampler-level inconsistency).
#
#    Goal of this simulation:
#      - Validate on toy data (known truth) that v11t recovers K_true = 4
#        comparably to v11, while keeping the b_pm geometry less compressed
#        (item piling toward mu_c reduced) and PSM contrast comparable.
#      - Inspect per-item w_q distribution: items at cluster boundary should
#        get w_q < 1 (downweighted), items near cluster centers w_q ~ 1+.
# =========================================================
data_dir <- "/Users/todoo/Desktop/학교/대학원/Research/joint_LSIRM/data"
proj_dir <- "/Users/todoo/Desktop/학교/대학원/Research/joint_LSIRM"
setwd(data_dir)
source(file.path(data_dir, "my_LSIRM_FMC_cpp_v11t.R"))  # joint LSIRM + t-mixture v11t
...
# Option B: isotropic clusters with slight size variation.
# Removes the 6:1 anisotropy of the original v10 simulation that the
# default isotropic NIW prior (S0 ~ I) cannot represent well.  Per-cluster
# sd ranges 0.32..0.45, distance between centers = 2, so center/sd ratio
# is ~ 5 -> clean Gaussian-mixture-friendly separation while still leaving
# size variation for S0 ~ Wishart hyperprior to learn from data.
sigma_meta <- list(
  diag(c(0.15, 0.15)),
  diag(c(0.20, 0.20)),
  diag(c(0.10, 0.10)),
...
plot_dir <- file.path(
  proj_dir, "plot",
  sprintf("simulation_4_layered_v11t_d%d_S0_%g_nu%g", d, S0_scale, 4)
)
...
# =========================================================
# 11) Run v11t joint MCMC (Student-t mixture, nu_t = 4)
# =========================================================
nu_t_fit <- 4

cat(sprintf(
  "\n========== simulation_4_layered_v11t (n=%d, P=%d/%d/%d/%d, K_true=%d, K*=%d, M_SM=%d, S0_scale=%g, nu_t=%g) ==========\n",
  n, P1, P2, P3, P4, K_true, K_star, n_split_merge, S0_scale, nu_t_fit))
...
result <- lsirm_fmc_v11t_cpp(
  ...
  # v11t Student-t mixture df.  Default 4 -- moderate-heavy tail to relax
  # b geometric pull while preserving cluster identification at center.
  # nu_t -> Inf reduces to v11 Gaussian mixture; nu_t in [3, 10] is the
  # recommended range.
  nu_t          = nu_t_fit,
  verbose       = TRUE,
  fix_gamma     = FALSE,
...
# =========================================================
# 22) v11t-specific: Student-t weight (w_q) diagnostics
#
#   For each item q we have a posterior trace of w_q over saved iterations.
#   Items at cluster boundary should have w_q distributed below 1 (Gamma
#   mass shifted left), items near cluster center should sit around 1 or
#   slightly above.  We plot:
#     - per-item posterior mean E[w_q] coloured by true cluster
#     - per-item w_q trace (a few examples)
#     - histogram of E[w_q] across items
#   And save E[w_q] alongside the partition so paper text can describe
#   which items the t-mixture downweighted.
# =========================================================
if (!is.null(samps$fmc_ws)) {
  ws_mat   <- samps$fmc_ws                        # n_save x P_total
  ws_pmean <- colMeans(ws_mat)
  ws_q025  <- apply(ws_mat, 2, quantile, 0.025)
  ws_q975  <- apply(ws_mat, 2, quantile, 0.975)

  pdf(file.path(plot_dir, "v11t_ws_posterior_mean.pdf"), width = 10, height = 6)
  par(mar = c(8, 4, 3, 1))
  ord_w  <- order(ws_pmean)
  pal_w  <- expand_pal(K_true, cluster_pal)
  cols_w <- pal_w[true_item_cluster][ord_w]
  plot(seq_along(ws_pmean), ws_pmean[ord_w], pch = 19, cex = 0.8,
       col = cols_w,
       ylim = range(c(ws_q025, ws_q975)),
       xlab = "", ylab = "posterior E[w_q]",
       main = sprintf("v11t Student-t weight posterior (sorted; nu_t=%g)", nu_t_fit),
       xaxt = "n")
  ...
  axis(1, at = seq_along(ws_pmean),
       labels = item_names_full[ord_w], las = 2, cex.axis = 0.45)
  legend("topleft",
         legend = c(sprintf("True cluster %d", seq_len(K_true)), "w_q = 1 (Gaussian-equiv)"),
         col    = c(pal_w, "gray40"),
         pch    = c(rep(19, K_true), NA),
         lty    = c(rep(NA, K_true), 2),
         bty = "n", cex = 0.7)
  dev.off()

  pdf(file.path(plot_dir, "v11t_ws_hist.pdf"), width = 7, height = 5)
  par(mar = c(4, 4, 3, 1))
  hist(ws_pmean, breaks = 30, col = "steelblue", border = "white",
       main = sprintf("Posterior E[w_q] across %d items (nu_t=%g)",
                      length(ws_pmean), nu_t_fit),
       xlab = "E[w_q]")
  abline(v = 1, lty = 2, col = "red", lwd = 2)
  dev.off()

  # Save per-item summary for paper text.
  write.csv(data.frame(
            ...
            file.path(plot_dir, "v11t_ws_per_item.csv"),
            row.names = FALSE)
...
cat(sprintf("\n-> v11t simulation plots & artifacts saved to: %s\n", plot_dir))
cat("\n=== Done! ===\n")
