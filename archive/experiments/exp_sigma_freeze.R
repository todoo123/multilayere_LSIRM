## Hypothesis test: is the v12 PSM-uniformity caused by sigma being too free?
##
## Same data-generating mechanism as simulation_4_layered_v12_2.R (seed 20260501,
## n=150, P=120, K_true=4, alpha=1 tau=1 fixed).  Three sigma regimes:
##   X1  cluster-aligned sigma_init + n_perm_swaps = 0   (sigma frozen at GT)
##   X2  random         sigma_init  + n_perm_swaps = 0   (sigma frozen at random) -- control
##   X3  cluster-aligned sigma_init + n_perm_swaps = 10  (slow updates)           -- compromise
##
## Reports per regime: sigma swap rate, split/merge rate, K_+ posterior,
## within/between PSM contrast, and hclust/Dahl/Binder/VI ARI.
## A short chain (n_iter = 20000, burnin = 8000, thin = 5) suffices to detect
## the effect because the v12_2 baseline already showed it at this length.
...
## ===== same data-gen as simulation_4_layered_v12_2.R =====
set.seed(20260501)
n  <- 150
...
## ===== EPA-init helper (PCA proxy + k-means c init, only sigma differs) =====
...
cluster_aligned_sigma  <- order(true_item_cluster)               ## 1-based; wrapper -> 0-based
random_sigma_seed_0502 <- { set.seed(0502); sample.int(P_total, P_total) }

## ===== Hypers / proposal sds (same as v12_2 sim file) =====
...
run_one <- function(label, sigma_init_1based, n_perm_swaps_use,
                    n_iter=20000, burnin=8000, thin=5) {
  cat(sprintf("\n========== Run %s (n_perm_swaps=%d, sigma_init=%s) ==========\n",
              label, n_perm_swaps_use,
              if (identical(sigma_init_1based, cluster_aligned_sigma)) "cluster-aligned"
              else "random"))
  epa_init <- list(c=as.integer(km$cluster), sigma=as.integer(sigma_init_1based),
                   alpha=1.0, tau=1.0)
  set.seed(20260507)  ## same MCMC seed across regimes
  res <- lsirm_epa_v12_cpp(
    ...
    n_split_merge=1L,
    n_split_merge_R=5L,
    n_perm_swaps=as.integer(n_perm_swaps_use),
    b_epa_coupling=TRUE,
    update_alpha=FALSE,
    update_tau=FALSE,
    verbose=FALSE,
...
R_X1 <- run_one("X1_aligned_freeze", cluster_aligned_sigma,  0,  n_iter_use, burnin_use, thin_use)
R_X2 <- run_one("X2_random_freeze",  random_sigma_seed_0502, 0,  n_iter_use, burnin_use, thin_use)
R_X3 <- run_one("X3_aligned_slow",   cluster_aligned_sigma,  10, n_iter_use, burnin_use, thin_use)

## ===== Print summary table =====
cat("\n\n================== SUMMARY ==================\n")
print_one <- function(R) {
  cat(sprintf("[%s]\n", R$label))
  cat(sprintf("  sigma_swap_rate=%.3f  split_rate=%.3f  merge_rate=%.3f\n",
              R$sigma_swap_rate, R$split_rate, R$merge_rate))
  cat(sprintf("  K_+: median=%g  mean=%.2f\n", R$median_K, R$mean_K))
...
saveRDS(list(X1=R_X1, X2=R_X2, X3=R_X3),
        file.path(proj_dir, "exp_sigma_freeze_results.rds"))
cat("\nResults saved to exp_sigma_freeze_results.rds\n")
