## v12 EPA delta-sweep
##
## Hypothesis: α-sweep (exp_alpha_sweep.R) showed PSM contrast can be recovered
## by relaxing the new-cluster prior penalty, but α↑ blows up K_+ uniformly
## across all K (over-splitting at α=5/10).  The DDT (2017) Pitman-Yor-style
## discount δ ∈ [0, 1) instead changes the *shape* of the K-prior: the
## new-cluster factor (α + δ q_{t-1})/(α + t-1) makes opening a *higher* K
## progressively easier (compounds in q_{t-1}).  Theoretical prediction:
##   - K=1 → K=2 transition gets only modest boost (×(1 + δ/α))
##   - K=2 → K=3, K=3 → K=4 get progressively bigger boost
##   - Sweet spot may exist where K=1 collapse is escaped without K explosion
##
## Setting: SAME data-gen as exp_alpha_sweep.R (seed 20260501, P=120, K_true=4).
## Free σ (M_perm = P_total), τ=1 fixed, α=1 fixed.  δ ∈ {0, 0.25, 0.5, 0.75}.
...
run_one <- function(label, delta_fix,
                    n_iter=20000, burnin=8000, thin=5) {
  cat(sprintf("\n========== Run %s (alpha_fix=1, tau_fix=1, delta_fix=%g, free sigma) ==========\n",
              label, delta_fix))
  ...
  ## Per-iter cluster-size geometry (Task 7 style: largest-share, HHI)
  ...
  list(label=label, delta_fix=delta_fix,
...
R_d00 <- run_one("delta=0.00", 0.00, n_iter_use, burnin_use, thin_use)
R_d25 <- run_one("delta=0.25", 0.25, n_iter_use, burnin_use, thin_use)
R_d50 <- run_one("delta=0.50", 0.50, n_iter_use, burnin_use, thin_use)
R_d75 <- run_one("delta=0.75", 0.75, n_iter_use, burnin_use, thin_use)
...
print_one <- function(R) {
  ...
  cat(sprintf("  Largest-share: mean=%.3f  median=%.3f   HHI mean=%.3f  K_eff mean=%.2f\n",
              R$largest_share_mean, R$largest_share_median,
              R$hhi_mean, R$k_eff_mean))
  cat(sprintf("  %% iters with largest>0.95 (K=1-like): %.1f%%   >0.85: %.1f%%\n",
              100*R$pct_K1_like, 100*R$pct_collapsed))
...
saveRDS(list(d00=R_d00, d25=R_d25, d50=R_d50, d75=R_d75),
        file.path(proj_dir, "exp_delta_sweep_results.rds"))
cat("\nResults saved to exp_delta_sweep_results.rds\n")
