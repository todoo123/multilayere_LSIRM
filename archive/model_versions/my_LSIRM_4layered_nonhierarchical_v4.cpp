...
// =========================================================
// Main MCMC Function (4-layered, independent gamma per layer)
// =========================================================
// [[Rcpp::export]]
List run_lsirm_cpp(
  ...
  vec u(P4, fill::zeros);  // u fixed at 0
  ...
  for(int iter = 0; iter < n_iter; ++iter) {
    ...
    if (fix_gamma == true){
      acc_log_gamma1++; acc_log_gamma2++; acc_log_gamma3++; acc_log_gamma4++;
    } else {
    ...
    // --- 6. Update Ordinal Thresholds (u fixed at 0, update delta only) ---
    for(int j=0; j<P4; ++j) {
    ...
    // Reconstruct thresholds with u = 0 fixed
