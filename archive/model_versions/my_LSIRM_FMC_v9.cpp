...
// =========================================================
// v9: Joint multilayered LSIRM (5-layer, per-item kappa)
//     + Probabilistic-PCA-style mixture clustering (PPCA-FMC)
//
//   Key change vs v8: the FA layer's per-respondent residual
//   variance psi_i (vector of length n) is replaced by a single
//   shared scalar sigma_eps_sq (homoscedastic noise). This
//   converts the FA layer into Probabilistic PCA (Tipping &
//   Bishop, 1999) with a Gaussian-mixture prior on item factor
//   scores. The change removes the Lambda->0 degenerate fixed
//   point that crippled v8 cluster recovery: a single residual
//   variance cannot selectively absorb respondent-specific
//   variance, forcing Lambda*eta to carry the cross-respondent
//   structure.
//
//   All other blocks (LSIRM, mixture in eta, delta, sigma_delta_sq)
//   are unchanged from v8.
// =========================================================
// v8 (for reference): Joint multilayered LSIRM (5-layer, per-item kappa)
...
List run_lsirm_fmc_v9_cpp(
  ...
  // PPCA noise: single shared variance sigma_eps_sq ~ IG(a_eps, b_eps).
  // Replaces v8's per-respondent psi_i ~ IG(a_psi, b_psi).
  ...
  // PPCA noise variance (single scalar): full trace always saved (cheap).
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    //          -> (delta_i, lambda_i) per i -> sigma_eps_sq -> sigma_delta_sq
    //
    ...
    // PPCA: Lambda^T (1/sigma_eps_sq) Lambda = (1/sigma_eps_sq) * Lambda^T Lambda
    ...
    for (int j = 0; j < P_total; ++j) {
      ...
      vec wr   = (x_log.col(j) - delta) * inv_sigma_eps_sq; // n-vec
    ...
    // C6/C7. delta_i, lambda_i  (per-respondent Gibbs scan, using shared sigma_eps_sq)
    // C8.   sigma_eps_sq (single scalar Gibbs after the loop, on total RSS)
    ...
    for (int i = 0; i < n; ++i) {
      ...
      // Accumulate residual SS (uses NEW delta_i, lambda_i) for C8
    ...
    // C8. sigma_eps_sq | rest  -- single Gibbs draw from IG.
    //     Conjugate posterior:
    //       sigma_eps_sq | . ~ IG(a_eps + n*P/2,  b_eps + total_rss/2)
    {
