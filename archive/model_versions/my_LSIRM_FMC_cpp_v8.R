...
# v8: joint LSIRM (5-layer, per-item kappa) + Factor-Mixture Clustering (FMC)
sourceCpp(file.path(getwd(), "my_LSIRM_FMC_v8.cpp"))

# =========================================================
# lsirm_fmc_v8_cpp(): wrapper around run_lsirm_fmc_v8_cpp.
#
#   - LSIRM block is COPIED VERBATIM from v6 inside the .cpp.
#     Do NOT modify it; the user requires strict one-way coupling
#     LSIRM -> FMC (clustering must NOT influence LSIRM updates).
#
#   - Each MCMC iteration runs all v6 LSIRM updates, then builds
#     x_{ij} = log max(||a_i - b_concat_j||, FMC_EPS) and runs one
#     FMC Gibbs sweep (factor-mixture clustering of items).
#
#   - LSIRM and FMC hyperparameters / inits are kept in *separate*
#     lists for readability; FMC has no proposal_sd because every
#     update is a closed-form Gibbs step.
#
...
lsirm_fmc_v8_cpp <- function(
    ...
    # ---- FMC init: list(rho, eta, c, mu, Sigma, Lambda, delta, psi, sigma_delta_sq) ----
    fmc_init = NULL,
    ...
    save_psi_full    = FALSE,   # n_save x n      -- LARGE, default OFF
  ...
  if (is.null(fmc_init)) {
    ...
    if (length(fmc_init$psi)   != n) stop("fmc_init$psi must have length n.")
  }
  ...
  # ----- Run C++ MCMC -----
  cat(sprintf(
    "Running joint LSIRM+FMC v8 [n=%d, P_total=%d (%d/%d/%d/%d/%d), r_fac=%d, K_star=%d, e0=%g, nu2=%g]\n",
    n, P_total, P1, P2, P3, P4, P5, r_fac, K_star, e0, nu2
  ))
  res <- run_lsirm_fmc_v8_cpp(
    Y_bin, Y_con, Y_cnt, Y_ord1, Y_ord2,
