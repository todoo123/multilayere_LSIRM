...
# v11tm: v11t with the OPTIONAL collapsed-label (marginalized) b update.
#
#   Adds a single switch:
#     marginal_b = TRUE  -> the b_q MH ratio uses the leave-one-out
#                           NIW Student-t mixture predictive (model_v11
#                           eq.(b-mh-ratio-marginal); partially collapsed
#                           Gibbs, van Dyk & Park 2008).  Same joint
#                           posterior as v11t with the conditional kernel.
#     marginal_b = FALSE -> v11t default (current-label conditional MH).
#
#   Everything else is byte-identical to v11t (Student-t mixture via
#   Gaussian scale-mixture, weighted NIW posteriors, Jain-Neal SM,
#   per-item w_q Gamma update).
sourceCpp(file.path(
  "/Users/hyunseokyoon/Desktop/학교/대학원/Research/joint_LSIRM/data",
  "my_LSIRM_FMC_v11tm.cpp"
))

# =========================================================
# lsirm_fmc_v11tm_cpp(): wrapper around run_lsirm_fmc_v11tm_cpp.
# =========================================================
lsirm_fmc_v11tm_cpp <- function(
    ...
    # v11t: Student-t mixture degrees of freedom.  Default 4 (heavy-tailed,
    # robust to LSIRM-geometry items at cluster boundaries).  As nu_t -> Inf
    # this reduces to v11 Gaussian mixture; as nu_t -> 2 the variance becomes
    # infinite.  Recommended range: [3, 10].  This is FIXED -- not estimated
    # from data -- to keep the model identifiable.  This parameter REPLACES
    # the v11 b_prior_inflation knob, which was a sampler-level non-coherent
    # workaround for the same biplot-distortion symptom.
    nu_t = 4,

    # v11tm: collapsed-label (marginalized) b update toggle.  Default TRUE
    # (the new sampler).  When TRUE, the b_q MH ratio integrates
    # (c_q, rho, mu, Sigma) out of the prior factor and uses
    #   A(b; q, w_q) = log sum_l (n_{l,-q} + e0)
    #                          * t_{nu_n - d + 1}(b; m_n, V_pred(w_q))
    # in place of the single-cluster Gaussian penalty.  Same stationary
    # distribution as v11t (partially collapsed Gibbs, van Dyk & Park 2008).
    # Set marginal_b = FALSE to recover the v11t kernel exactly for
  ...
  # ----- Run C++ MCMC -----
  cat(sprintf(
    "Running joint LSIRM + v11tm latent-position t-mixture [n=%d, P_total=%d (%d/%d/%d/%d/%d), d=%d, K_star=%d, e0=%g, kappa0=%g, M_SM=%d, marginal_b=%s]\n",
    n, P_total, P1, P2, P3, P4, P5, d, K_star, e0, fmc_hyper$kappa0,
    as.integer(n_split_merge), as.logical(marginal_b)
  ))
  res <- run_lsirm_fmc_v11tm_cpp(
    Y_bin, Y_con, Y_cnt, Y_ord1, Y_ord2,
  ...
  res$info <- list(
    ...
    fmc_kind = "latent_position_t_mixture_niw_split_merge",
    compute_co_cluster_online = compute_co_cluster_online,
    coupling = if (isTRUE(marginal_b))
                 "fully_joint_LSIRM_and_t_mixture_marginal_b"
               else "fully_joint_LSIRM_and_t_mixture",
    n_split_merge = as.integer(n_split_merge),
    nu_t          = as.numeric(nu_t),
    marginal_b    = as.logical(marginal_b),
