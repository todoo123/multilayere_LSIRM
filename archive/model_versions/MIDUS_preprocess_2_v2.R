...
biomarker_0001$B4H42

hist(biomarker_0001$B4SSQ_GS)
hist(survey_0001$B1SA54A)
hist(survey_0001$B1SRXMED)
...
#    (v2: right-skewed continuous 변수에 log 변환 추가)
################################################################################

# log 변환 대상: right-skewed biomarker / reaction time
...
biomarker_selected <- biomarker_selected %>%
  mutate(
    ...
    # log 변환: right-skewed biomarker (표준화 전 적용)
...
# ─── P3: Cognitive ────────────────────────────────────────────────────────────
# 역코딩 대상: Z-score 변수 (높을수록 인지기능 좋음 → 부호 반전하여 높을수록 인지저하 방향)
...
cognitive_selected <- cognitive_selected %>%
  mutate(
    ...
    # 역코딩: Z-score × (-1), 높을수록 인지기능 저하 방향
    across(all_of(cog_reverse_vars), ~ -.x),
    # log 변환: right-skewed reaction time (표준화 전 적용)
...
split_for_lsirm <- function(df, bin_vars, ord1_vars, ord2_vars, con_vars, label) {
  ...
  # row/col 매핑 + screening branch 정보 보존
  ...
  list(
    ...
    branch   = branch_info$branch,        # screening branch (Sadness/Anhedonia/Both)
