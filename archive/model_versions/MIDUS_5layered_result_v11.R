...
# 0506 update: pin Sigma_l via highly informative IW prior to break the
# data-driven Sigma_l shrinkage feedback that caused item piling at alpha=1.
#
# Mechanism of the previous piling:
#   prior pulls b -> mu_c => empirical within-cluster var shrinks
#   -> NIW posterior Sigma_l shrinks (S_n / (nu_n - d - 1))
#   -> b prior precision = 1/Sigma_l grows
#   -> tighter pull, runaway shrinkage, items pile on mu_c.
#
# Fix: choose (nu0, S0) so that E[Sigma_l|S0] is held near a target
# bandwidth ~ 0.25 I (per-dim sd ~ 0.5), comparable to v10 reference
# within-cluster spread.  Disable the Wishart hyperprior on S0 so S0
# itself does not adapt to the (already-shrunk) data.
#
#   nu0 = 100, d = 2 -> nu0 - d - 1 = 97
#   E[Sigma_l|S0=25*I] = 25 / 97 ~= 0.258 * I  (per-dim sd ~ 0.51)
#   mu_l|Sigma_l ~ N(0, Sigma_l/kappa0=1) -> sd ~ 0.51 -> cluster centers
#   span ~ +-1, mixture differentiation preserved.
#
#   With Sigma_l pinned at 0.258 I:
#     b prior precision per dim = 1/0.258 ~= 3.9
#     likelihood precision per b (e.g., Bernoulli, n=221, gamma~0.5):
#       ~ 0.25 * 0.5^2 * 221 ~= 14
#   -> likelihood >> prior, b spreads under data drive while still
#      anchored to cluster center.
# 0506 update 2: v10 biplot reference shows cluster visible radius ~ 0.5 in
# the d=2 latent space.  Reading that as the 2-sigma boundary of a 2D Gaussian
# gives target per-dim sd 0.25 (E[Sigma_l] = 0.0625 * I).  With nu0 = 100 and
# d = 2, denominator nu0 - d - 1 = 97, so S0_scale = 0.0625 * 97 ~= 6.
#
# kappa0 must drop below 1 in tandem: with Sigma_l ~ 0.0625 I, kappa0 = 1
# would force mu_l prior sd to 0.25 (cluster centers crammed within +/- 0.5).
