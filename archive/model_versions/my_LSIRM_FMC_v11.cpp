...
// =========================================================
// v11: Joint multilayered LSIRM (5-layer, per-item kappa)
//      + Latent-position Gaussian mixture clustering
//      + Conjugate NIW prior on (mu_l, Sigma_l) in R^d
//      + Collapsed (NIW posterior-predictive) c_q Gibbs update
//      + Jain-Neal restricted-Gibbs split-merge moves
//
//   Key change vs v10 (the only change to the model):
//     The PPCA measurement layer is removed.  The mixture is
//     placed DIRECTLY on the item latent positions:
//       z_q := b_{j(q)}^{(l(q))} in R^d,
//       z_q | c_q = l, mu_l, Sigma_l ~ N_d(mu_l, Sigma_l).
//     b_j updates therefore include the mixture prior log-
//     density in their MH ratio (FULLY JOINT coupling), instead
//     of the v10 N(0, I_d) prior on b.  No eta, Lambda, delta,
//     sigma_eps_sq, sigma_delta_sq, row_center.
//
//   The LSIRM block (alpha, beta, gamma, a, b-likelihood,
//   beta4_thr, beta5_thr, log_kappa, sigma0_sq,
//   sigma_alpha_l_sq, tau_beta_l_sq=4) is kept BYTE-IDENTICAL
//   to v10, except that the b update prior term changes from
//     -0.5 * dot(b, b)
//   to
...
List run_lsirm_fmc_v11_cpp(
    ...
    int  fmc_warmup,
    int  n_split_merge,
    double b_prior_inflation
) {
  // Cluster-mixture-prior inflation factor used ONLY in the b MH update.
  //
  //   b ~ N_d(mu_{c_q}, alpha * Sigma_{c_q})    where alpha = b_prior_inflation
  //
  // alpha = 1: full coupling (paper-defined joint posterior)
  // alpha > 1: weaker pull on b -> better LSIRM geometry recovery
  // alpha = +Inf: no pull -> equivalent to v10 one-way coupling
  //
  // The c, mu, Sigma updates use the original Sigma (alpha = 1), so cluster
  // identity is unaffected; only b's MH ratio sees the inflated covariance.
  // This decouples the geometry-anchoring strength from the cluster-shape
  // estimation strength. It is a sampler-level approximation, not a full
  // Bayesian model change.
  if (b_prior_inflation <= 0.0)
    Rcpp::stop("b_prior_inflation must be > 0");
  double inv_b_inflation = 1.0 / b_prior_inflation;
  if (verbose) {
    Rcout << "[v11 b prior inflation] alpha = " << b_prior_inflation
          << "  (1/alpha = " << inv_b_inflation << ")\n";
  }
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    for (int j = 0; j < P1; ++j) {
      ...
      if (log(R::runif(0,1)) <
          (ll_f(b_prop) - 0.5 * inv_b_inflation * q_new -
           (ll_f(b_old) - 0.5 * inv_b_inflation * q_old))) {
    ...
    //   z items currently in cluster l excluding q.
    {
      ...
      for (int q = 0; q < P_total; ++q) {
        ...
        for (int l = 0; l < K_star; ++l) {
          niw_posterior_from_indices(z_all, clusters_idx[l],
                                     m0, kappa0, nu0, S0,
      ...
      for (int sm = 0; sm < n_split_merge; ++sm) {
        ...
        if (c_label(u) == c_label(v)) {
          ...
          for (int q : scan_order) {
            niw_posterior_from_indices(z_all, tmp_a,
                                       m0, kappa0, nu0, S0,
                                       kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices(z_all, tmp_b,
                                       m0, kappa0, nu0, S0,
          ...
          niw_posterior_from_indices(z_all, A_idx, m0, kappa0, nu0, S0,
                                     kappa_n, nu_n, m_n, S_n);
          ...
          {
            niw_posterior_from_indices(z_all, tmp_a, m0, kappa0, nu0, S0,
                                       kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices(z_all, tmp_b, m0, kappa0, nu0, S0,
                                       kappa_b, nu_b, m_b, S_b);
          ...
          for (int q : scan_order) {
            ...
            niw_posterior_from_indices(z_all, rev_a,
                                       m0, kappa0, nu0, S0,
                                       kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices(z_all, rev_b,
                                       m0, kappa0, nu0, S0,
                                       kappa_b, nu_b, m_b, S_b);
          ...
          double log_pi_old = 0.0;
          {
            niw_posterior_from_indices(z_all, A_idx, m0, kappa0, nu0, S0,
                                       kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices(z_all, B_idx, m0, kappa0, nu0, S0,
                                       kappa_b, nu_b, m_b, S_b);
            double log_m_A = log_niw_marginal_from_post(n_A, d, kappa0, nu0,
                                                       log_det_S0, log_mvg_nu0_half,
                                                       kappa_a, nu_a, S_a);
          ...
          double log_pi_new = 0.0;
          {
            ...
            niw_posterior_from_indices(z_all, AB_idx, m0, kappa0, nu0, S0,
                                       kappa_n, nu_n, m_n, S_n);
