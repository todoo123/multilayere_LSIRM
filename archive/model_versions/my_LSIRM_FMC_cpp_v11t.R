...
# v11t: joint LSIRM (5-layer, per-item kappa)
#       + Latent-position Student-t MIXTURE clustering
#         (Gaussian scale-mixture form, Lange-Little-Taylor 1989):
#           b_q | c_q = l ~ t_nu(mu_l, Sigma_l)
#         hierarchical:
#           w_q ~ Gamma(nu/2, nu/2)
#           b_q | c_q = l, w_q ~ N_d(mu_l, Sigma_l / w_q)
#       + conjugate NIW prior on (mu_l, Sigma_l) with WEIGHTED
#         sufficient statistics (kappa_n = kappa0 + sum w, etc.)
#       + Collapsed (Student-t predictive) c_q Gibbs update
#       + Jain-Neal split-merge with weighted scatter
#
#   Differences from v11:
#     * Mixture prior changes from Gaussian to Student-t
#       (variance inflation through per-item w_q ~ Gamma).
#     * Removes the b_prior_inflation knob (which was a
#       sampler-level non-coherent hack).  v11t replaces it
#       with a proper Bayesian Student-t mixture.
#     * Adds nu_t parameter (Student-t df, default 4).
#     * Returns extra fmc_ws (n_save x P_total) of weights.
sourceCpp(file.path(getwd(), "my_LSIRM_FMC_v11t.cpp"))

# =========================================================
# lsirm_fmc_v11t_cpp(): wrapper around run_lsirm_fmc_v11t_cpp.
# =========================================================
lsirm_fmc_v11t_cpp <- function(
  ...
  res$info <- list(
    ...
    coupling = "fully_joint_LSIRM_and_t_mixture",
