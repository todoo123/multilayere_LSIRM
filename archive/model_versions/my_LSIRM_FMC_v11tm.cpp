...
// =========================================================
// v11tm: Joint multilayered LSIRM + t-MIXTURE clustering with
//        OPTIONAL collapsed-label (marginalized) b update
//        (Student-t mixture via Gaussian scale-mixture form)
//
//   This file is v11t with one additional sampler option:
//   marginal_b = TRUE switches the b_q MH update from the
//   single-cluster conditional prior penalty (v11t default) to
//   the leave-one-out NIW Student-t mixture predictive,
//   marginalizing (c_q, rho, mu, Sigma) at the b_q step.  This
//   is a partially collapsed Gibbs move (van Dyk & Park 2008),
//   and corresponds to model_v11_latent_position_mixture.tex
//   eq.(b-mh-ratio-marginal).  The joint posterior is
//   unchanged; only the b-update kernel differs.
//
//   v11 used a Gaussian mixture: b_q | c_q = l ~ N_d(mu_l, Sigma_l).
//   v11t replaces this with a Student-t mixture
//       b_q | c_q = l ~ t_nu(mu_l, Sigma_l)
//   represented hierarchically (Lange-Little-Taylor 1989) as
//       w_q | c_q = l ~ Gamma(nu/2, nu/2)            (per-item weight)
//       b_q | c_q = l, w_q ~ N_d(mu_l, Sigma_l / w_q).
//   nu (Student-t df) is FIXED (default 4) -- not estimated.
//
//   Why: the Gaussian mixture's quadratic prior penalty pulled
//   b_q strongly toward mu_{c_q} for items at cluster boundaries,
//   compressing the LSIRM biplot.  Student-t has logarithmic
//   tail decay so far-from-center items get tolerated --
//   geometry recovers without giving up cluster identification.
//
//   Conditional on w_q the model is exactly the v11 Gaussian
//   mixture rescaled by w_q.  The conjugate NIW updates carry
//   over with WEIGHTED sufficient statistics:
//       W_l   = sum_{q in l} w_q                (effective n)
//       zbar_w = (sum_{q in l} w_q z_q) / W_l    (weighted mean)
//       S_w    = sum_{q in l} w_q (z_q - zbar_w) (z_q - zbar_w)'
//       kappa_n = kappa0 + W_l                  (NIW posterior)
//       nu_n    = nu0    + n_l                  (unchanged form)
//       m_n     = (kappa0 m0 + W_l zbar_w) / kappa_n
//       S_n     = S0 + S_w + (kappa0 W_l)/(kappa0 + W_l)
//                              * (zbar_w - m0)(zbar_w - m0)'.
//   Predictive density at item q with weight w_q (collapsed
//   c-Gibbs and Jain-Neal split-merge) is multivariate t with
//       dof   = nu_n - d + 1
//       scale = ((kappa_n + w_q) / (kappa_n * w_q * dof)) * S_n.
//   The Pi w_q^{d/2} factor from each Gaussian density is
//   constant in any partition move (same set of items) and
//   cancels in the MH ratio.
//
//   Per-sweep updates added on top of v11:
//     - w_q ~ Gamma((nu+d)/2, (nu + Delta_q)/2) where
//       Delta_q = (b_q - mu_{c_q})' Sigma_{c_q}^{-1} (b_q - mu_{c_q}).
//   Per-sweep updates modified:
//     - b_q MH ratio: prior penalty 0.5 (b - mu_c)' Sigma_c^{-1} (...) -> 0.5 w_q (...)
//     - c_q Gibbs:     V_pred uses (kappa_n + w_q) / (kappa_n * w_q * dof)
//     - Split-merge:   weighted niw_posterior_from_indices
//     - (mu, Sigma):   weighted niw_posterior_from_indices
//
//   The LSIRM block (alpha, beta, gamma, a, b-likelihood,
//   beta4_thr, beta5_thr, log_kappa, sigma0_sq,
//   sigma_alpha_l_sq, tau_beta_l_sq=4) is BYTE-IDENTICAL to
//   v11; only the FMC block touches w_q.
// =========================================================
...
// v11tm: per-cluster sufficient-statistics cache for the marginal-b prior.
//
//   W_l       = sum_{q in l} w_q                (effective n)
//   sum_wz_l  = sum_{q in l} w_q z_q            (d-vector; column l)
//   sum_wzz_l = sum_{q in l} w_q z_q z_q'       (d x d; slice l)
//   n_l       = |{q : c_q = l}|                 (count; component l)
//
// From these the weighted NIW posterior (kappa_n, nu_n, m_n, S_n) can be
// recovered in O(d^2) without iterating over cluster members, and the
// leave-one-out variant (l = c_q) is a constant-time subtraction of q's
// contribution.  The cache is rebuilt once at the start of each b sweep
// (rebuilds also reset roundoff accumulated by incremental updates) and
// incrementally maintained inside the sweep on every accepted MH move.
static inline void build_cluster_stats_cache(
    const mat& z_all, const uvec& c_label, const vec& ws,
    int K_star, int d,
    vec& W_cache, mat& sum_wz_cache, cube& sum_wzz_cache, ivec& n_cache
) {
  W_cache.zeros();
  sum_wz_cache.zeros();
  sum_wzz_cache.zeros();
  n_cache.zeros();
  int P = (int) z_all.n_rows;
  for (int q = 0; q < P; ++q) {
    int    l  = (int) c_label(q);
    double wq = ws[q];
    vec    zq = z_all.row(q).t();
    W_cache(l)             += wq;
    sum_wz_cache.col(l)    += wq * zq;
    sum_wzz_cache.slice(l) += wq * (zq * zq.t());
    n_cache(l)             += 1;
  }
  (void)d;
}

// v11tm: weighted NIW posterior from the cluster-stats cache, with
// optional leave-one-out subtraction when l == cq.  Mathematically
// identical to niw_posterior_from_indices_weighted(z, cluster_l \ {q})
// but evaluated in O(d^2) using the cached sufficient statistics.
//
//   zbar_w = sum_wz / W
//   S_w    = sum_wzz - W zbar_w zbar_w'      (uncentered form;
//                                              the S0 floor in S_n
//                                              dominates any roundoff)
static inline void niw_posterior_loo_from_cache(
    int l, int q, int cq,
    const mat& z_all, const vec& ws,
    const vec& W_cache, const mat& sum_wz_cache, const cube& sum_wzz_cache,
    const ivec& n_cache,
    const vec& m0, double kappa0, double nu0, const mat& S0,
    int d,
    double& kappa_n, double& nu_n, vec& m_n, mat& S_n
) {
  double W_l    = W_cache(l);
  vec    sumwz  = sum_wz_cache.col(l);
  mat    sumwzz = sum_wzz_cache.slice(l);
  int    n_l    = n_cache(l);
  ...
  vec zbar_w = sumwz / W_l;
  mat S_w    = sumwzz - W_l * (zbar_w * zbar_w.t());
  S_w        = 0.5 * (S_w + S_w.t());
  ...
  m_n     = (kappa0 * m0 + W_l * zbar_w) / kappa_n;
  vec dvec = zbar_w - m0;
  double w_eff = (kappa0 * W_l) / kappa_n;
  S_n = S0 + S_w + w_eff * (dvec * dvec.t());
  S_n = 0.5 * (S_n + S_n.t());
}

// v11tm: log marginalized mixture-prior factor for the b update,
// using cached cluster sufficient statistics + leave-one-out.
//
//   A(b; q, w_q) = log sum_l (n_{l,-q} + e0)
//                          * t_{dof_l}(b; m_{l,-q}, V_pred_l(w_q))
//
// where (kappa_n, nu_n, m_n, S_n) are computed by
// niw_posterior_loo_from_cache.  Empty clusters contribute the NIW
// prior predictive at (m0, S0) with weight e0.
//
// Replaces the O(K_star * P_total) full-recompute version with an
// O(K_star * d^2) lookup; correctness is identical because the cache
// is exact (it is rebuilt at the start of each b sweep and updated
// incrementally on accept).
static inline double log_marginal_b_prior_cached(
    const vec& b_val, int q, double w_q, int cq,
    const mat& z_all, const vec& ws,
    const vec& W_cache, const mat& sum_wz_cache, const cube& sum_wzz_cache,
    const ivec& n_cache,
    const vec& m0, double kappa0, double nu0, const mat& S0,
    int K_star, int d, double e0
) {
  vec lw(K_star);
  vec m_n(d);
  mat S_n(d, d);
  ...
  for (int l = 0; l < K_star; ++l) {
    niw_posterior_loo_from_cache(l, q, cq, z_all, ws,
                                 W_cache, sum_wz_cache, sum_wzz_cache, n_cache,
                                 m0, kappa0, nu0, S0, d,
                                 kappa_n, nu_n, m_n, S_n);
    int n_lmq = (l == cq) ? (n_cache(l) - 1) : n_cache(l);
    double dof = nu_n - (double) d + 1.0;
    mat V_pred = ((kappa_n + w_q) / (kappa_n * w_q * dof)) * S_n;
    lw(l) = std::log((double) n_lmq + e0)
          + log_mvt_density(b_val, m_n, V_pred, dof);
  }
  double M = lw.max();
  double s = arma::sum(arma::exp(lw - M));
  return M + std::log(s);
}

// =========================================================
// Main MCMC (v11tm: t-mixture + optional marginal_b)
// =========================================================
// [[Rcpp::export]]
List run_lsirm_fmc_v11tm_cpp(
  ...
  // v11t Student-t mixture df.  nu_t = Inf would reduce to v11 Gaussian
  // mixture; nu_t in [3, 10] gives heavy- to moderately-heavy-tailed
  // robust clustering.  Default 4 (matches LSIRM continuous-layer Student-t).
  if (!(nu_t > 2.0))
    Rcpp::stop("nu_t (Student-t df) must be > 2 (variance finite).");
  if (verbose) {
    Rcout << "[v11t Student-t mixture] nu_t = " << nu_t << "\n";
  }
  ...
  // v11tm marginal-b cluster-stats cache (only populated when marginal_b).
  // Built once per iteration before the b sweep and incrementally updated
  // on accepted MH moves; cluster Gibbs / split-merge updates do not need
  // to touch it (rebuilt fresh next iteration).
  vec  W_cache(K_star, fill::zeros);
  mat  sum_wz_cache(d, K_star, fill::zeros);
  cube sum_wzz_cache(d, d, K_star, fill::zeros);
  ivec n_cache(K_star, fill::zeros);
  ...
  // v11t: per-item Student-t scale weights (Gamma-distributed latent variables).
  // Initialised to 1 (Gaussian-equivalent start); updated each sweep from the
  // Gamma full conditional after b is updated.
  vec ws(P_total, fill::ones);

  // Mixture-prior quadratic form: (b - mu_cl)^T Sigma_cl^{-1} (b - mu_cl).
  auto mix_quad = [&](const rowvec& b_val, int cl) {
    rowvec dv = b_val - mu_mat.row(cl);
    return arma::as_scalar(dv * SigInv.slice(cl) * dv.t());
  };

  // v11tm: when marginal_b is on, the b update needs z_all and clusters_idx
  // synced at the START of the b block.  Prime them from the FMC init so the
  // first iteration's b update has a valid leave-one-out predictive.  After
  // that, z_all is maintained incrementally inside the b loops; clusters_idx
  // is rebuilt from c_label inside the FMC block as before.
  if (marginal_b) {
    ...
    if (verbose)
      Rcout << "[v11tm] marginal_b = TRUE  (collapsed-label b update)\n";
  } else if (verbose) {
    Rcout << "[v11tm] marginal_b = FALSE (current-label conditional b update; same as v11t)\n";
  }
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    // ----------------------------------------------------------------
    // 5. b layer updates.  Two MH kernels are available for the prior
    //    log-difference; the LSIRM likelihood part is identical.
    //
    //    marginal_b == false  (v11t default):
    //        prior diff = -0.5 * w_q * [(b'-mu_c)' SigInv_c (b'-mu_c)
    //                                  -(b -mu_c)' SigInv_c (b -mu_c)]
    //
    //    marginal_b == true   (v11tm marginalized):
    //        prior diff = A(b'; q, w_q) - A(b; q, w_q)
    //        where A is the leave-one-out NIW mixture-predictive
    //        log-sum-exp (see log_marginal_b_prior_cached above).
    //
    //    When marginal_b == true we maintain z_all in sync with the
    //    current b layers so the leave-one-out NIW posterior in
    //    log_marginal_b_prior_cached sees the latest item positions.
    //    Cluster sufficient statistics (W, sum_wz, sum_wzz, n) are
    //    rebuilt once here from the current (z_all, c_label, ws) -- the
    //    FMC block in the previous iteration may have changed c and ws
    //    -- and then incrementally updated on every accepted b move so
    //    the next b_q's prior evaluation sees the post-accept cluster
    //    stats.  Cluster membership c_label does not change during the
    //    b sweep, so the (l != c_q) caches stay leave-one-out by
    //    construction; only cluster c_q's stats are touched on accept.
    // ----------------------------------------------------------------

    if (marginal_b) {
      build_cluster_stats_cache(z_all, c_label, ws, K_star, d,
                                W_cache, sum_wz_cache, sum_wzz_cache, n_cache);
    }
    ...
    for (int j = 0; j < P1; ++j) {
      ...
      double prior_old, prior_new;
      if (marginal_b) {
        vec b_old_v  = b_old.t();
        vec b_prop_v = b_prop.t();
        prior_old = log_marginal_b_prior_cached(b_old_v,  q, ws[q], cl,
                                                z_all, ws,
                                                W_cache, sum_wz_cache,
                                                sum_wzz_cache, n_cache,
                                                m0, kappa0, nu0, S0,
                                                K_star, d, e0);
        prior_new = log_marginal_b_prior_cached(b_prop_v, q, ws[q], cl,
                                                z_all, ws,
                                                W_cache, sum_wz_cache,
                                                sum_wzz_cache, n_cache,
                                                m0, kappa0, nu0, S0,
        ...
        prior_old = -0.5 * ws[q] * mix_quad(b_old,  cl);
        prior_new = -0.5 * ws[q] * mix_quad(b_prop, cl);
      }
      if (log(R::runif(0,1)) <
        ...
        if (marginal_b) {
          ...
          vec b_old_v  = b_old.t();
          vec b_prop_v = b_prop.t();
          sum_wz_cache.col(cl)    += wq * (b_prop_v - b_old_v);
          sum_wzz_cache.slice(cl) += wq * (b_prop_v * b_prop_v.t()
                                          - b_old_v * b_old_v.t());
        }
      }
    }
    ...
    //   z items currently in cluster l excluding q.
    {
      ...
      for (int q = 0; q < P_total; ++q) {
        ...
        vec z_q = z_all.row(q).t();
        vec lw(K_star);

        for (int l = 0; l < K_star; ++l) {
          niw_posterior_from_indices_weighted(z_all, clusters_idx[l], ws,
                                              m0, kappa0, nu0, S0,
                                              kappa_n, nu_n, m_n, S_n);
          int n_lmq = (int)clusters_idx[l].size();
          double dof = nu_n - (double)d + 1.0;
          // v11t: predictive covariance scales by w_q (the weight that the
          // candidate item brings into cluster l).  When w_q = 1 this reduces
          // exactly to v11.
          double wq = ws[q];
          mat V_pred = ((kappa_n + wq) / (kappa_n * wq * dof)) * S_n;
          double log_w = std::log((double)n_lmq + e0)
                       + log_mvt_density(z_q, m_n, V_pred, dof);
          lw(l) = log_w;
        }
      ...
      for (int sm = 0; sm < n_split_merge; ++sm) {
        ...
        if (c_label(u) == c_label(v)) {
          ...
          for (int q : scan_order) {
            niw_posterior_from_indices_weighted(z_all, tmp_a, ws,
                                                m0, kappa0, nu0, S0,
                                                kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices_weighted(z_all, tmp_b, ws,
                                                m0, kappa0, nu0, S0,
            ...
            double lw_a = std::log((double)n_a + e0)
                        + log_mvt_density(z_q, m_a, V_a, dof_a);
            double lw_b = std::log((double)n_b + e0)
                        + log_mvt_density(z_q, m_b, V_b, dof_b);
          ...
          niw_posterior_from_indices_weighted(z_all, A_idx, ws, m0, kappa0, nu0, S0,
                                              kappa_n, nu_n, m_n, S_n);
          ...
          {
            niw_posterior_from_indices_weighted(z_all, tmp_a, ws, m0, kappa0, nu0, S0,
                                                kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices_weighted(z_all, tmp_b, ws, m0, kappa0, nu0, S0,
                                                kappa_b, nu_b, m_b, S_b);
          ...
          for (int q : scan_order) {
            ...
            niw_posterior_from_indices_weighted(z_all, rev_a, ws,
                                                m0, kappa0, nu0, S0,
                                                kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices_weighted(z_all, rev_b, ws,
                                                m0, kappa0, nu0, S0,
                                                kappa_b, nu_b, m_b, S_b);
          ...
          double log_pi_old = 0.0;
          {
            niw_posterior_from_indices_weighted(z_all, A_idx, ws, m0, kappa0, nu0, S0,
                                                kappa_a, nu_a, m_a, S_a);
            niw_posterior_from_indices_weighted(z_all, B_idx, ws, m0, kappa0, nu0, S0,
                                                kappa_b, nu_b, m_b, S_b);
            double log_m_A = log_niw_marginal_from_post(n_A, d, kappa0, nu0,
                                                       log_det_S0, log_mvg_nu0_half,
                                                       kappa_a, nu_a, S_a);
          ...
          double log_pi_new = 0.0;
          {
            ...
            niw_posterior_from_indices_weighted(z_all, AB_idx, ws, m0, kappa0, nu0, S0,
                                                kappa_n, nu_n, m_n, S_n);
    ...
    // C3. (mu_l, Sigma_l) | rest  -- NIW posterior draw (weighted, v11t)
    {
      vec m_n(d);
      mat S_n(d, d);
      double kappa_n, nu_n;
    ...
    // C3a. v11t: per-item Student-t weights w_q | b, c, mu, Sigma
    //
    //   w_q ~ Gamma(alpha_w, beta_w)
    //     alpha_w = (nu_t + d) / 2
    //     beta_w  = (nu_t + Delta_q) / 2
    //     Delta_q = (b_q - mu_{c_q})' Sigma_{c_q}^{-1} (b_q - mu_{c_q})
    //
    //   E[w_q | ...] = (nu_t + d) / (nu_t + Delta_q)
    //
    // Items close to their cluster center get w_q ~ 1+ (Gaussian-like),
    // items far from the center get w_q << 1 (downweighted -> heavy tail).
    {
      double alpha_w = 0.5 * (nu_t + (double)d);
      for (int q = 0; q < P_total; ++q) {
        int cl = (int) c_label(q);
        rowvec dv = z_all.row(q) - mu_mat.row(cl);
        double Delta_q = arma::as_scalar(dv * SigInv.slice(cl) * dv.t());
        double beta_w  = 0.5 * (nu_t + Delta_q);
        // R::rgamma uses shape-scale parameterisation; scale = 1/rate.
