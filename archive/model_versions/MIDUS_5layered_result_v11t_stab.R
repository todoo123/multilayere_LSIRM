...
################################################################################
setwd(data_dir)
source(file.path(data_dir, "my_LSIRM_FMC_cpp_v11t.R"))  # joint LSIRM + Student-t mixture v11t
...
d      <- 2L
K_star <- 6L                       # 0509 stabilization: was 10; reduces label-switching mode count from K_star! ~ 3.6M to 720
e0     <- 0.05
...
common_fmc_hyper <- list(
  e0     = e0,
  m0     = rep(0, d),
  kappa0 = 1.0,                          # 0509 stabilization: was 0.1 (loose); tightened to reduce mu_l wandering
...
nu2 <- 20      # 0509 stabilization: was 4 (heavy-tail t for outlier robustness); raised to make continuous-layer likelihood near-Gaussian -> tighter b2 posterior, less trace multimodality (trade-off: less robust to inflammation outliers)
...
# 4. Single chain run
################################################################################
run_label     <- sprintf("v11t_STAB_d%d_K%d_e%g_S0init_%g_nu%d_kap%g_M%d_nut%g_nu2%g",
                         d, K_star, e0,
                         S0_init_scale,
