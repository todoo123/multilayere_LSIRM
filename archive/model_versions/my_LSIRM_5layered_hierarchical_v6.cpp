...
// =========================================================
// Main MCMC Function (5-layered hierarchical: bin, con(robust), cnt, ord1-GRM, ord2-GRM)
// =========================================================
// [[Rcpp::export]]
List run_lsirm_v6_cpp(
  ...
  // Hierarchical global-local linkage hyperparameters
  double a_sigma_global = hyper["a_sigma_global"];
  double b_sigma_global = hyper["b_sigma_global"];
  ...
  double sd_a1 = prop_sd["a1"];
  double sd_a2 = prop_sd["a2"];
  double sd_a3 = prop_sd["a3"];
  double sd_a4 = prop_sd["a4"];
  double sd_a5 = prop_sd["a5"];
  ...
  // Hierarchical respondent positions
  mat z  = as<mat>(init["z"]);   // n x d (global)
  mat a1 = as<mat>(init["a1"]);  // n x d (local, binary)
  mat a2 = as<mat>(init["a2"]);  // n x d (local, continuous)
  mat a3 = as<mat>(init["a3"]);  // n x d (local, count)
  mat a4 = as<mat>(init["a4"]);  // n x d (local, ordinal1)
  mat a5 = as<mat>(init["a5"]);  // n x d (local, ordinal2)
  double sigma1_sq = as<double>(init["sigma1_sq"]);  // global-local coupling variance
  ...
  vec store_sigma1_sq(n_save);

  cube store_z(n, d, n_save);
  cube store_a1(n, d, n_save);
  cube store_a2(n, d, n_save);
  cube store_a3(n, d, n_save);
  cube store_a4(n, d, n_save);
  cube store_a5(n, d, n_save);
  ...
  vec acc_a1 = zeros<vec>(n);
  vec acc_a2 = zeros<vec>(n);
  vec acc_a3 = zeros<vec>(n);
  vec acc_a4 = zeros<vec>(n);
  vec acc_a5 = zeros<vec>(n);
  ...
  for(int iter = 0; iter < n_iter; ++iter) {
    ...
    for(int i=0; i<n; ++i) {
      for(int j=0; j<P2; ++j) {
        double dist = calc_dist(a2.row(i), b2.row(j));
    ...
    for(int i=0; i<n; ++i) {
      ...
      auto ll_f = [&](double val) {
        double ll = 0.0;
        for(int j=0; j<P1; ++j) {
          double dist = calc_dist(a1.row(i), b1.row(j));
    ...
    for(int i=0; i<n; ++i) {
      ...
      auto ll_f = [&](double val) {
        double ll = 0.0;
        for(int j=0; j<P3; ++j) {
          double dist = calc_dist(a3.row(i), b3.row(j));
    ...
    for(int i=0; i<n; ++i) {
      ...
      auto ll_f = [&](double val) {
        double ll = 0.0;
        for(int j=0; j<P4; ++j) {
          double dist = calc_dist(a4.row(i), b4.row(j));
    ...
    for(int i=0; i<n; ++i) {
      ...
      auto ll_f = [&](double val) {
        double ll = 0.0;
        for(int j=0; j<P5; ++j) {
          double dist = calc_dist(a5.row(i), b5.row(j));
    ...
    // --- 4. Update Local Positions a_l[i] (Layer-specific MH with hierarchical prior) ---
    // a1 (Binary layer)
    for(int i=0; i<n; ++i) {
      rowvec pos_old = a1.row(i);
      rowvec pos_prop = pos_old + randn<rowvec>(d) * sd_a1;

      auto ll_f = [&](const rowvec& pos) {
        double ll = 0;
        for(int j=0; j<P1; ++j) {
      ...
      // Prior: a1[i] ~ N(z[i], sigma1_sq * I)
      double lp_curr = -0.5 * sum(square(pos_old - z.row(i))) / sigma1_sq;
      double lp_next = -0.5 * sum(square(pos_prop - z.row(i))) / sigma1_sq;

      if(log(R::runif(0,1)) < (ll_f(pos_prop) + lp_next - ll_f(pos_old) - lp_curr)) {
        a1.row(i) = pos_prop; acc_a1(i)++;
      }
    }

    // a2 (Continuous robust layer)
    for(int i=0; i<n; ++i) {
      rowvec pos_old = a2.row(i);
      rowvec pos_prop = pos_old + randn<rowvec>(d) * sd_a2;
      ...
      if(log(R::runif(0,1)) < (ll_f(pos_prop) + lp_next - ll_f(pos_old) - lp_curr)) {
        a2.row(i) = pos_prop; acc_a2(i)++;
      }
    }

    // a3 (Count layer)
    for(int i=0; i<n; ++i) {
      rowvec pos_old = a3.row(i);
      rowvec pos_prop = pos_old + randn<rowvec>(d) * sd_a3;
      double size = 1.0/kappa;
      ...
      if(log(R::runif(0,1)) < (ll_f(pos_prop) + lp_next - ll_f(pos_old) - lp_curr)) {
        a3.row(i) = pos_prop; acc_a3(i)++;
      }
    }

    // a4 (Ordinal1 GRM layer)
    for(int i=0; i<n; ++i) {
      rowvec pos_old = a4.row(i);
      rowvec pos_prop = pos_old + randn<rowvec>(d) * sd_a4;
      ...
      if(log(R::runif(0,1)) < (ll_f(pos_prop) + lp_next - ll_f(pos_old) - lp_curr)) {
        a4.row(i) = pos_prop; acc_a4(i)++;
      }
    }

    // a5 (Ordinal2 GRM layer)
    for(int i=0; i<n; ++i) {
      rowvec pos_old = a5.row(i);
      rowvec pos_prop = pos_old + randn<rowvec>(d) * sd_a5;
    ...
    for(int j=0; j<P1; ++j) {
      ...
      auto ll_f = [&](const rowvec& b_val) {
        double ll=0;
        for(int i=0; i<n; ++i){
          double dist = calc_dist(a1.row(i), b_val);
          double eta = alpha1(i) - beta1(j) - gamma_val1 * dist;
          ll += Y_bin(i,j)*eta - log1pexp(eta);
        }
        return ll;
    ...
    for(int j=0; j<P2; ++j) {
      ...
      auto ll_f = [&](const rowvec& b_val) {
        double ll=0;
        for(int i=0; i<n; ++i){
          double dist = calc_dist(a2.row(i), b_val);
          double mu = alpha2(i) - beta2(j) - gamma_val2 * dist;
          double lam = lambda2(i,j);
          double sd_eff = sqrt(sigma0_sq / lam);
          ll += R::dnorm(Y_con(i,j), mu, sd_eff, 1);
    ...
    for(int j=0; j<P3; ++j) {
      ...
      auto ll_f = [&](const rowvec& b_val) {
        double ll=0;
        for(int i=0; i<n; ++i){
          double dist = calc_dist(a3.row(i), b_val);
          double mu = exp(alpha3(i) - beta3(j) - gamma_val3 * dist);
    ...
    for(int j=0; j<P4; ++j) {
      ...
      auto ll_f = [&](const rowvec& b_val) {
        double ll=0;
        for(int i=0; i<n; ++i){
          double dist = calc_dist(a4.row(i), b_val);
          double eta = alpha4(i) - gamma_val4 * dist;
          ll += log_grm_prob_y0based(Y_ord1_0(i,j), beta4_thr.row(j), eta);
        }
        return ll;
    ...
    for(int j=0; j<P5; ++j) {
      ...
      auto ll_f = [&](const rowvec& b_val) {
        double ll=0;
        for(int i=0; i<n; ++i){
          double dist = calc_dist(a5.row(i), b_val);
          double eta = alpha5(i) - gamma_val5 * dist;
          ll += log_grm_prob_y0based(Y_ord2_0(i,j), beta5_thr.row(j), eta);
        }
        return ll;
    ...
    // --- 8b. Update Global Position z and sigma1_sq (Gibbs) ---
    {
      double var_z_post = 1.0 / (1.0 + 5.0 / sigma1_sq);
      double sd_z_post = sqrt(var_z_post);

      for(int i=0; i<n; ++i) {
        for(int k=0; k<d; ++k) {
          double sum_a = a1(i,k) + a2(i,k) + a3(i,k) + a4(i,k) + a5(i,k);
          double mean_z_post = var_z_post * sum_a / sigma1_sq;
          z(i,k) = R::rnorm(mean_z_post, sd_z_post);
        }
      }

      // sigma1_sq ~ InvGamma posterior
      double sse_z = 0.0;
      for(int i=0; i<n; ++i) {
        for(int k=0; k<d; ++k) {
          double d1 = a1(i,k) - z(i,k);
          double d2 = a2(i,k) - z(i,k);
          double d3 = a3(i,k) - z(i,k);
          double d4 = a4(i,k) - z(i,k);
          double d5 = a5(i,k) - z(i,k);
          sse_z += d1*d1 + d2*d2 + d3*d3 + d4*d4 + d5*d5;
        }
      }
      double total_obs = n * 5.0 * d;
      double shape = a_sigma_global + total_obs / 2.0;
      double rate = b_sigma_global + 0.5 * sse_z;
    ...
    if (iter >= burnin && (iter - burnin) % thin == 0) {
      ...
      store_sigma1_sq(save_idx) = sigma1_sq;

      store_z.slice(save_idx) = z;
      store_a1.slice(save_idx) = a1;
      store_a2.slice(save_idx) = a2;
      store_a3.slice(save_idx) = a3;
      store_a4.slice(save_idx) = a4;
      store_a5.slice(save_idx) = a5;
  ...
  return List::create(
    ...
    Named("sigma1_sq")      = store_sigma1_sq,
    ...
    Named("accept") = List::create(
      ...
      Named("a1")        = acc_a1 / n_iter,
      Named("a2")        = acc_a2 / n_iter,
      Named("a3")        = acc_a3 / n_iter,
      Named("a4")        = acc_a4 / n_iter,
      Named("a5")        = acc_a5 / n_iter,
