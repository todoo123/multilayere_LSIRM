...
# v12_skewt: SDB (Sahu-Dey-Branco 2003) skew-t mixture extension of v11t.
#
#   Each cluster's component is a skew-t:
#       w_q ~ Gamma(nu_t/2, nu_t/2),
#       U_q | w_q ~ N_+(0, 1/w_q),
#       b_q | c=l, w_q, U_q ~ N_d(mu_l + Delta_l * U_q, Sigma_l / w_q).
#
#   Delta_l in R^d is the per-cluster SKEW DIRECTION (Delta_l = 0 recovers
#   v11t exactly).  Prior:  Delta_l ~ N_d(0, tau_Delta^2 * I).  Reduces to
#   v11t (symmetric t mixture) as tau_Delta -> 0.
#
#   Conjugate updates carry over with residual r_q = b_q - Delta_{c_q} * U_q
#   replacing b_q in v11t's weighted-NIW machinery.  Additional Gibbs
#   blocks per sweep: Delta_l (multivariate Gaussian regression) and U_q
#   (truncated half-normal).  w_q Gibbs gets shape += 1/2, rate += U_q^2/2.
#
#   Conditional-b kernel only (no marginal_b).  v11tm's collapsed-label
#   marginal kernel can be added later if needed.
sourceCpp(file.path(
  "/Users/hyunseokyoon/Desktop/학교/대학원/Research/joint_LSIRM/data",
  "my_LSIRM_FMC_v12_skewt.cpp"
))

# =========================================================
# lsirm_fmc_v12_skewt_cpp(): wrapper around run_lsirm_fmc_v12_skewt_cpp.
# =========================================================
lsirm_fmc_v12_skewt_cpp <- function(
    ...
    # Student-t mixture degrees of freedom (FIXED, not estimated).  Default
    # 3 (heavy-tailed, robust to LSIRM-geometry items at cluster boundaries).
    # As nu_t -> Inf this reduces to a skew-NORMAL mixture; as nu_t -> 2 the
    # variance becomes infinite.  Recommended range: [3, 10].
    nu_t = 3,

    # v12 skew-t: prior sd for per-cluster skew direction Delta_l in R^d.
    #   Delta_l ~ N_d(0, tau_Delta^2 * I_d).
    # Larger tau_Delta -> more skew allowed; tau_Delta -> 0 collapses to v11t.
    # Default 1.0 -- moderate skew prior, matches typical LSIRM latent scale.
    tau_Delta = 1.0,
  ...
  # ----- Run C++ MCMC -----
  cat(sprintf(
    "Running joint LSIRM + v12 SDB skew-t mixture [n=%d, P_total=%d (%d/%d/%d/%d/%d), d=%d, K_star=%d, e0=%g, kappa0=%g, M_SM=%d, nu_t=%g, tau_Delta=%g]\n",
    n, P_total, P1, P2, P3, P4, P5, d, K_star, e0, fmc_hyper$kappa0,
    as.integer(n_split_merge), as.numeric(nu_t), as.numeric(tau_Delta)
  ))
  res <- run_lsirm_fmc_v12_skewt_cpp(
    Y_bin, Y_con, Y_cnt, Y_ord1, Y_ord2,
  ...
  apply_rigid_one_iter <- function(i, R_mat, trans_vec) {
    ...
    # v12 skew-t: Delta_l is a direction vector in R^d -- rotate but do
    # NOT translate.  Under x' = x R, b_q' = (mu_l + Delta_l U_q + ...) R
    # so Delta_l' = Delta_l R.  U_q is invariant (scalar latent).
    if (!is.null(res$fmc_Delta)) {
      D_old <- res$fmc_Delta[,,i]               # K_star x d
      res$fmc_Delta[,,i] <<- D_old %*% R_mat
    }
  }
  ...
  res$info <- list(
    ...
    fmc_kind = "latent_position_skewt_mixture_niw_split_merge",
    compute_co_cluster_online = compute_co_cluster_online,
    coupling = "fully_joint_LSIRM_and_skewt_mixture_SDB",
    n_split_merge = as.integer(n_split_merge),
    nu_t          = as.numeric(nu_t),
    tau_Delta     = as.numeric(tau_Delta),
