...
# v6: per-item kappa_j for the count layer
sourceCpp(file.path(getwd(), "my_LSIRM_5layered_nonhierarchical_v6.cpp"))

lsirm_sharedpos_layer5_grm_v6_cpp <- function(
    ...
    prop_sd = list(
      ...
      log_kappa=0.05   # shared proposal SD, applied per-item
    ),
    init = NULL,
    verbose = TRUE,
    fix_gamma = FALSE
  ...
  if (is.null(init)) {
    ...
    init <- list(
      ...
      # v6: per-item log_kappa_j, length P3 (empty vector if P3 == 0)
      log_kappa = rep(0, P3),
    ...
    # 길이 보정: scalar로 들어온 경우 P3 길이로 복제
    if (length(init$log_kappa) == 1 && P3 > 1) {
      init$log_kappa <- rep(as.numeric(init$log_kappa), P3)
    } else if (length(init$log_kappa) == 0 && P3 > 0) {
      init$log_kappa <- rep(0, P3)
    }
  }

  # --- Run C++ MCMC ---
  cat("Running C++ MCMC (v6: 5-layered, per-item kappa_j, GRM ordinal, robust continuous, nu2 =", nu2, ")...\n")
