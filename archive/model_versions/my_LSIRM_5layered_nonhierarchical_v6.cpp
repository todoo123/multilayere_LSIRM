...
// =========================================================
// v6 change from v5: per-item kappa_j for the count layer
//   - log_kappa: scalar → vec(P3)
//   - Each item j has its own kappa_j ~ LogNormal(mu_log_kappa, sd_log_kappa)
//     (shared prior hyperparameters, per-item parameters)
//   - size_j = 1 / kappa_j used in NB likelihood for item j
// =========================================================
...
// =========================================================
// Main MCMC Function (v6: per-item kappa_j for count layer)
// =========================================================
// [[Rcpp::export]]
List run_lsirm_v6_cpp(
  ...
  // Shared prior hyperparameters for per-item log_kappa_j (same as v5)
  ...
  double sd_log_kappa_prop = prop_sd["log_kappa"];   // shared proposal SD, used per item
  ...
  // v6: per-item log_kappa_j (length P3)
  ...
  // v6: store per-item log_kappa as matrix (n_save × P3)
  ...
  // v6: per-item kappa acceptance counter
  ...
  for(int iter = 0; iter < n_iter; ++iter) {
    ...
    // --- 0. Update latent scales lambda2 (Gibbs) ---
    for(int i=0; i<n; ++i) {
      for(int j=0; j<P2; ++j) {
    ...
    // Alpha 3 (Cnt) — v6: per-item size computed inside j loop
    for(int i=0; i<n; ++i) {
    ...
    // Beta 3 — v6: size uses kappa(j) for this j
    for(int j=0; j<P3; ++j) {
    ...
    if (fix_gamma == true){
      ...
      // --- 3c. Update Gamma3 (Cnt) — v6: per-item size_j ---
      {
    ...
    // --- 4. Update Shared Position a_i — v6: L3 uses per-item size_j ---
    for(int i=0; i<n; ++i) {
    ...
    // b3 — v6: size uses kappa(j) for this j
    for(int j=0; j<P3; ++j) {
    ...
    // --- 7. Log Kappa_j (v6: per-item MH with shared prior) ---
    for(int j=0; j<P3; ++j) {
  ...
  return List::create(
    ...
    // v6: log_kappa is now a matrix (n_save x P3)
    ...
    Named("accept") = List::create(
      ...
      // v6: per-item kappa acceptance vector (length P3)
