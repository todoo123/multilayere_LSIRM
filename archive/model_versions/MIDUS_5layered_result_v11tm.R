...
################################################################################
# v11tm: v11t_stab + collapsed-label (marginalized) b update
#
#   Same model and hyperparameters as MIDUS_5layered_result_v11t_stab.R
#   (the 0509 stabilization lock-in: K_star=6, kappa0=1.0, nu0=12,
#   S0_init_scale=1, nu_t=3, nu2=20, n_iter=30000, burnin=10000, thin=5).
#
#   ONLY change: the b_q MH update now marginalizes (c_q, rho, mu, Sigma)
#   via the leave-one-out NIW Student-t mixture predictive
#   (model_v11_latent_position_mixture.tex eq.(b-mh-ratio-marginal);
#   partially collapsed Gibbs, van Dyk & Park 2008).
#
#   Compare to v11t_stab to assess whether the marginalized b update
#   reduces LSIRM-geometry compression and/or improves cluster recovery.
################################################################################
...
################################################################################
setwd(data_dir)
source(file.path(data_dir, "my_LSIRM_FMC_cpp_v11tm.R"))  # joint LSIRM + Student-t mixture v11tm (marginal_b option)
...
common_fmc_hyper <- list(
  ...
  nu0    = 5,                           # 100 -> 10 (loose, data-driven Sigma_l)
...
common_mcmc <- list(d = d, n_iter = 30000, burnin = 10000, thin = 5)   # 0509 stabilization preview: was 120k/30k/10
...
##############################################################################
# 4-A. v11t joint MCMC (Student-t mixture)
##############################################################################
